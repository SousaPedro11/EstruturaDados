...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    private RedBlackNode<T> root = this.nil;
    ...
    private void leftRotate(final RedBlackNode<T> x) {

        // Call leftRotateFixup() which updates the numLeft
        // and numRight values.
        this.leftRotateFixup(x);
    ...
    private void leftRotateFixup(final RedBlackNode x) {

        // Case 1: Only x, x.right and x.right.right always are not nil.
        if (this.isNil(x.left) && this.isNil(x.right.left)) {
            x.numLeft = 0;
            x.numRight = 0;
            x.right.numLeft = 1;
        }
        ...
        else if (this.isNil(x.left) && !this.isNil(x.right.left)) {
            x.numLeft = 0;
    ...
    // Updates the numLeft and numRight values affected by the rotate
    private void rightRotateFixup(final RedBlackNode y) {

        // Case 1: Only y, y.left and y.left.left exists.
        if (this.isNil(y.right) && this.isNil(y.left.right)) {
            y.numRight = 0;
            y.numLeft = 0;
            y.left.numRight = 1;
        }
        ...
        else if (this.isNil(y.right) && !this.isNil(y.left.right)) {
            y.numRight = 0;
    ...
    private void insert(final RedBlackNode<T> z) {
    ...
    public void remove(final RedBlackNode<T> v) {

        final RedBlackNode<T> z = this.search(v.key);

        // Declare variables
        RedBlackNode<T> x = this.nil;
        RedBlackNode<T> y = this.nil;

        // if either one of z's children is nil, then we must remove z
        if (this.isNil(z.left) || this.isNil(z.right)) {
            y = z;
        } else {
        ...
        if (this.isNil(y.parent)) {
            this.root = x;
        } else if (!this.isNil(y.parent.left) && (y.parent.left == y)) {
            y.parent.left = x;
        } else if (!this.isNil(y.parent.right) && (y.parent.right == y)) {
            y.parent.right = x;
        }
    ...
    private void fixNodeData(final RedBlackNode<T> x, final RedBlackNode<T> y) {

        // Initialize two variables which will help us traverse the tree
        RedBlackNode<T> current = this.nil;
        RedBlackNode<T> track = this.nil;
        ...
        while (!this.isNil(current)) {
            ...
            // current node we are checking
                ...
                if (this.isNil(current.left)) {
                    current.numLeft--;
                } else if (this.isNil(current.right)) {
                    current.numRight--;
    ...
    public RedBlackNode<T> search(final T key) {
    ...
    public int findNumGreater(final RedBlackNode<T> node, final T key) {

        // Base Case: if node is nil, return 0
        if (this.isNil(node)) {
            return 0;
        } else if (key.compareTo(node.key) < 0) {
            return 1 + node.numRight + this.findNumGreater(node.left, key);
        } else {
            return this.findNumGreater(node.right, key);
        }
     ...
     */
    public List<T> getGreaterThan(final T key, final Integer maxReturned) {

        final List<T> list = new ArrayList<>();
        this.getGreaterThan(this.root, key, list);
        return list.subList(0, Math.min(maxReturned, list.size()));
    }

    private void getGreaterThan(final RedBlackNode<T> node, final T key,
                    final List<T> list) {

        if (this.isNil(node)) {
            return;
        } else if (node.key.compareTo(key) > 0) {
            this.getGreaterThan(node.left, key, list);
            list.add(node.key);
            this.getGreaterThan(node.right, key, list);
        } else {
            this.getGreaterThan(node.right, key, list);
        }
    }
    ...
    public int findNumSmaller(final RedBlackNode<T> node, final T key) {

        // Base Case: if node is nil, return 0
        if (this.isNil(node)) {
            return 0;
        } else if (key.compareTo(node.key) <= 0) {
            return this.findNumSmaller(node.left, key);
        } else {
            return 1 + node.numLeft + this.findNumSmaller(node.right, key);
        }
    ...
    private boolean isNil(final RedBlackNode node) {
    ...
    public int size() {
        ...
        return this.root.numLeft + this.root.numRight + 1;
    }// end size()
